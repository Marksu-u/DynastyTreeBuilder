...
describe('buildOrderingUnits', () => {
  const build = (fix: { nodes: LayoutNodeIn[]; edges: LayoutEdgeIn[] }) => {
...
describe('orderLayers', () => {
  const prep = (fix: { nodes: LayoutNodeIn[]; edges: LayoutEdgeIn[] }) => {
    const g = buildFamilyGraph(fix.nodes, fix.edges);
    const r = assignGenerations(g);
...
describe('assignX', () => {
  const run = (fix: { nodes: LayoutNodeIn[]; edges: LayoutEdgeIn[] }) => {
    const g = buildFamilyGraph(fix.nodes, fix.edges);
    const r = assignGenerations(g);
