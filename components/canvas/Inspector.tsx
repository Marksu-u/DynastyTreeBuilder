...
const GENDERS: CharacterGender[] = ['UNKNOWN', 'MALE', 'FEMALE', 'NON_BINARY'];

const CHARACTER_FLAGS: CharacterFlag[] = [
...
function subjectLabel(
  ...
  switch (mode.kind) {
    case 'edit': return mode.character.data.name || tr('subject.edit');
    case 'create': return tr('subject.create');
    case 'relative':
      return tr('subject.relative', { kind: tr(`kinds.${mode.relative}`) });
  }
}
...
export function Inspector({
  ...
  const tr = useTranslations('canvas.inspector');
  ...
  const title =
    active.kind === 'edit' ? (active.character.data.name || tr('title.unnamed'))
      : active.kind === 'create' ? tr('title.create')
        : tr('title.relative', { kind: tr(`kinds.${active.relative}`) });

  const subtitle = active.kind === 'relative'
    ? tr('subtitle', { name: active.anchor.data.name })
    : null;
      ...
      {pendingMode && (
        ...
        >
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="text-[13px] font-medium text-zinc-100">{tr('unsaved.title')}</p>
              <p className="mt-1.5 text-[12px] leading-relaxed text-zinc-400">
                {tr('unsaved.body', {
                  subject: subjectLabel(active, tr),
                  target:
                    pendingMode.kind === 'edit'
                      ? (pendingMode.character.data.name || tr('target.another'))
                      : pendingMode.kind === 'create'
                        ? tr('target.create')
                        : tr('target.relative', {
                            kind: tr(`kinds.${pendingMode.relative}`),
                          }),
                })}
              </p>
            </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4">
          {linking && active.kind === 'relative' ? (
            <>
              ...
              <Field label={tr('fields.search')}>
                <input
                  autoFocus
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder={tr('fields.searchPlaceholder')}
                  className={FIELD}
                />
              </Field>
              {candidates.length === 0 ? (
            ...
            <>
              <Field label={isEdit ? tr('fields.name') : tr('fields.nameRequired')}>
                <input
                  autoFocus={!isEdit}
                  required
              ...
              <Field label={tr('fields.alias')}>
                <input
                  maxLength={MAX_ALIAS}
                  value={form.alias ?? ''}
                  onChange={(e) => set('alias', e.target.value)}
                  placeholder={tr('fields.aliasPlaceholder')}
                  className={FIELD}
                />
              </Field>

              <div className="flex gap-2.5">
                <Field label={tr('fields.role')} className="min-w-0 flex-1">
                  <input
                    value={form.style}
                    onChange={(e) => set('style', e.target.value)}
                    placeholder={tr('fields.rolePlaceholder')}
                    className={FIELD}
                  />
                </Field>
                <Field label={tr('fields.gender')} className="min-w-0 flex-1">
                  <select
                    value={form.gender}
                  ...
                  >
                    {GENDERS.map((g) => (
                      <option key={g} value={g}>{tr(`genders.${g}`)}</option>
                    ))}
                  </select>
                </Field>
              </div>

              <Field label={tr('fields.flags')}>
                <div className="flex flex-wrap gap-1.5">
                  {CHARACTER_FLAGS.map((value) => {
                    const label = tr(`flags.${value}`);
            ...
            <>
              <LinkGroup
                icon={<Heart size={11} />}
                label={tr('fields.partners')}
                addLabel={tr('add.partner')}
              ...
              <LinkGroup
                ...
                label={tr('fields.parents')}
                addLabel={tr('add.parent')}
              ...
              <LinkGroup
                icon={<Baby size={11} />}
                label={tr('fields.children')}
                addLabel={tr('add.child')}
            ...
            <Field label={tr('fields.plotHook')}>
              <textarea
                maxLength={MAX_NOTE}
                value={form.note ?? ''}
                onChange={(e) => set('note', e.target.value)}
                placeholder={tr('fields.plotHookPlaceholder')}
                rows={8}
...
function UnionChoice({
  ...
  return (
    <Field label={tr('fields.whichPartner')}>
...
function LinkGroup({
  ...
  return (
    <section className="flex flex-col gap-1.5">
      ...
      {people.length === 0 ? (
        <p className="text-[11px] text-zinc-600">{tr('none')}</p>
      ) : (
        <ul className="flex flex-col gap-0.5">
          {people.map((p) => (
            <li key={p.id}>
              ...
              >
                <span className={`truncate ${p.isGhost ? 'italic' : ''}`}>
                  {p.isGhost ? tr('unknown') : p.name}
                </span>
                {p.adopted && (
                  <span className="shrink-0 text-[10px] text-success">{tr('adopted')}</span>
                )}
              </button>
            </li>
          ))}
