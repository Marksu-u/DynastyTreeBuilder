...
function getFriendlyErrorMessage(err: unknown): string {
  ...
  // `catch` binds unknown, and a rejected Server Action can throw a non-Error.
  // Read the message defensively rather than asserting a shape we don't have.
...
export function DynastyCanvas({
  ...
  const performSave = useCallback(
    async <T,>(action: () => Promise<T>, successMessage?: string, errorMessage?: string): Promise<T> => {
      ...
      try {
        ...
        if (errorMessage) {
          toast.error(t('toasts.saveErrorDetail', { message: errorMessage, error: friendlyError }));
        } else {
          toast.error(t('toasts.failedToSave', { error: friendlyError }));
        }
        throw err;
      }
    },
  ...
  const handleUpdateCharacter = useCallback(
    async (data: CharacterData) => {
      ...
      if (provisionalIds.has(id)) {
        toast.info(t('toasts.stillSaving'));
        return;
      }
      const prevNodes = nodes;
      setNodes((nds) =>
    ...
    [editingCharacterId, dynastyId, nodes, performSave, provisionalIds, t]
  );
  ...
  }, [editingCharacterId, dynastyId, nodes, edges, performSave, provisionalIds, t]);
  ...
  const handleAddRelative = useCallback(async (input: AddRelativeInput) => {
    ...
    if (!result.ok) {
      toast.error(result.error === 'AMBIGUOUS_UNION' ? t('toasts.pickPartner') : result.error);
      return;
    }
    setRelPicker(null);
  ...
  }, [nodes, edges, dynastyId, performSave, markProvisional, t]);
  ...
  const handleExportJson = useCallback(async () => {
    ...
    } catch { toast.error(t('toasts.exportFailed')); }
  }, [dynastyId, dynastyName, t]);
  ...
  const runImport = useCallback(async (file: File) => {
    ...
    try {
      ...
      toast.error(t('toasts.importFailed', { error: getFriendlyErrorMessage(err) }));
    }
  }, [dynastyId, router, t]);
    ...
    <CanvasContext.Provider value={{ setEditingCharacterId: openEdit, openAddRelative }}>
      ...
      <div ref={containerRef} className={`relative flex-1 min-w-0 h-full ${settlingClass}`}>
        ...
        <ConfirmDialog
          ...
          title={t('replaceDynasty.title', { name: dynastyName })}
          description={t('replaceDynasty.description')}
          confirmLabel={t('replaceDynasty.confirmLabel')}
          destructive
