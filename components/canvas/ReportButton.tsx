...
/** Stored values. Their labels live in the `canvas.report.reasons` namespace. */
const REASONS = [
  "ILLEGAL_CONTENT",
  "HATE_SPEECH",
  "SPAM",
...
export function ReportButton({ shareSlug }: Props) {
  const t = useTranslations("canvas.report");
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="dialog-content fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-zinc-700 bg-zinc-900 p-6 shadow-xl">
          <div className="mb-4 flex items-center justify-between">
            <Dialog.Title className="text-sm font-semibold text-zinc-100">
              {t("title")}
            </Dialog.Title>
            <Dialog.Close className="text-zinc-500 hover:text-zinc-300" aria-label={tCommon("close")}>
              <X className="h-4 w-4" />
            </Dialog.Close>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              >
                <Select.Trigger className="flex w-full items-center justify-between rounded-md border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-500">
                  <Select.Value placeholder={t("reasonPlaceholder")} />
                  <Select.Icon>
                  ...
                  <Select.Content className="z-[60] overflow-hidden rounded-md border border-zinc-700 bg-zinc-800 shadow-xl">
                    <Select.Viewport className="p-1">
                      {REASONS.map((r) => (
                        ...
                        >
                          <Select.ItemText>{t(`reasons.${r}`)}</Select.ItemText>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-zinc-400">
                {t("details")}{" "}
                <span className="text-zinc-600">{t("detailsHint")}</span>
              </label>
              <textarea
                value={details}
