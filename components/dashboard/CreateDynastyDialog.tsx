        ...
        <Dialog.Content className="dialog-content fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border border-zinc-800 bg-background p-6 shadow-xl">
          <div className="mb-5 flex items-center justify-between">
            ...
            <Dialog.Close asChild>
              <button aria-label={t("close")} className="rounded p-1 text-zinc-500 hover:text-zinc-300">
                <X className="h-4 w-4" />
              </button>
            </Dialog.Close>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Select.Content className="z-50 overflow-hidden rounded-md border border-zinc-700 bg-zinc-900 shadow-xl">
                    <Select.Viewport className="p-1">
                      {SETTINGS.map((s) => (
                        ...
                        >
                          <Select.ItemText>{tSetting(s)}</Select.ItemText>
                        </Select.Item>
                      ))}
                    </Select.Viewport>
                  </Select.Content>
            ...
            >
              {isPending ? tNew("creating") : tNew("create")}
            </button>
          </form>
        </Dialog.Content>
      </Dialog.Portal>
